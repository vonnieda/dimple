...
pub struct DirectoryMusicLibrary {
    path: String,
}

impl DirectoryMusicLibrary {
...
impl MusicLibrary for DirectoryMusicLibrary {
    ...
    fn releases(&self) -> Vec<Arc<Release>> {
        ...
        for file_entry in files {
            let file = File::open(file_entry.path()).unwrap();
            ...
            let format = symphonia::default::get_probe().format(&hint, media_source_stream, &fmt_opts, &meta_opts);
            match format {
                Ok(mut probed) => {
                    print_format(file_entry.path().to_str().unwrap(), &mut probed);
                },
                Err(err) => {
                    println!("{}", err);
                }
