...
pub fn track_details(url: &str, app: &App) {
    ...
    std::thread::spawn(move || {        
        ...
        let mut track: Track = librarian.get(&Track {
        ...
        ui.upgrade_in_event_loop(move |ui| {
            ...
            let genres: Vec<LinkAdapter> = track.genres.iter().cloned().map(|genre| {
                LinkAdapter {
            ...
            let mut links: Vec<LinkAdapter> = track.recording.links.iter().map(|link| {
                    LinkAdapter {
                        name: link.into(),
                        url: link.into(),
                    }
