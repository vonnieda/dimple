...
pub fn artist_details(url: &str, app: &App) {
    ...
    std::thread::spawn(move || {        
        ...
        let artist: Artist = librarian.get(key);
        ...
        ui.upgrade_in_event_loop(move |ui| {
            ...
            let genres: Vec<LinkAdapter> = artist.genres.iter().cloned().map(|genre| {
                LinkAdapter {
...
fn genre_links(genre: &Genre) -> Vec<LinkAdapter> {
    todo!()
}

fn link_adapters(links: HashSet<String>) -> Vec<LinkAdapter> {
    links.iter().map(|link| {
        LinkAdapter {
            name: link.into(),
            url: link.into(),
